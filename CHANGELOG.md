## v2.2.0
- Headroom integration: every `install` checks for the [Headroom](https://github.com/headroomlabs-ai/headroom) compression proxy and prints setup steps. `models.yml` routing changes only via explicit opt-in: the install prompt or in-session `/headroom on` (prepare-only path, same reversible backup contract as `headroom wrap omp`).
...
- Interactive installs offer to enable Headroom routing on the spot (prepare-only wrap, never a nested launch); `reinstall` and `doctor` cover Headroom, and `update` inherits it via the latest installer.
